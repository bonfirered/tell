...
$(document).ready(function(){
	...
	$('#wall').facebookWall({
		id:'ahelpfulperson',
		access_token:'383273151765445|GMi3WFkHvH92lA-NqJxjTvIFqgk'
	});
	...
	$.fn.facebookWall = function(options){
		...
		$.when($.getJSON(graphUSER),$.getJSON(graphPOSTS)).done(function(user,posts){
			...
			$.each(posts[0].data,function(){
				...
				this.created_time = dt.getMonth()+1 + '.' + dt.getDate() + '.' + dt.getFullYear() ;
				
				//this.created_time = relativeTime(this.created_time*1000);
				
				// Converting URL strings to actual hyperlinks:
				
				if (this.message.length >= 190)
				{
					this.message = this.message.substring(0,190);
					
					//lastIndexOf()
					//alert("trim fb message");
					var lastspace = this.message.lastIndexOf(' ');
					this.message = this.message.substring(0,lastspace);
					this.message += "...";
